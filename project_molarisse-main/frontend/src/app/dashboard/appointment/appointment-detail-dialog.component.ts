      ...
      <mat-dialog-content>
        ...
        <div *ngIf="!isLoading">
          <div class="patient-info">
            ...
            <button mat-raised-button color="primary" (click)="viewPatientFiche()" class="view-fiche-btn">
...
export class AppointmentDetailDialogComponent {
  ...
  viewPatientFiche(): void {
    if (!this.appointment?.doctor?.id) {
      console.error('Doctor ID missing:', this.appointment?.doctor);
      this.snackBar.open('Erreur: ID du médecin manquant', 'Fermer', { duration: 3000 });
      return;
    }

    if (!this.appointment?.patient?.id) {
      console.error('Patient ID missing:', this.appointment?.patient);
      this.snackBar.open('Erreur: ID du patient manquant', 'Fermer', { duration: 3000 });
      return;
    }

    console.log('Opening fiche patient with:', {
      patientId: this.appointment.patient.id,
      doctorId: this.appointment.doctor.id
    });

    this.dialog.open(FichePatientComponent, {
      width: '80%',
      height: '90vh',
      data: {
        patientId: this.appointment.patient.id,
